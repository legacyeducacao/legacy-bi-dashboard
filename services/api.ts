...
export const fetchDashboardData = async (): Promise<DashboardData> => {
   ...
   try {
      ...
      const kpis: Record<string, MetricData> = {
         investment: { id: 'investment', label: 'Investimento', description: 'Gasto total em Meta Ads (API Meta Marketing). Fonte: Meta Graph API /insights', value: totalCost, goal: 200000, unit: 'currency' },
         leads: { id: 'leads', label: 'Leads', description: 'Total de formulários preenchidos (Ads + Orgânico). Fonte: Supabase → formularios_anuncios', value: totalLeads, goal: 800, unit: 'number' },
         leadsAds: { id: 'leadsAds', label: 'Leads Ads', description: 'Formulários com utm_source = Facebook/Google. Fonte: formularios_anuncios', value: leadsAds, goal: 600, unit: 'number' },
         leadsOrganic: { id: 'leadsOrganic', label: 'Leads Orgânico', description: 'Formulários com utm_source = organico/direto (Instagram, Reels, Bio). Fonte: formularios_anuncios', value: leadsOrganic, goal: 200, unit: 'number' },
         cpl: { id: 'cpl', label: 'CPL', description: 'Custo por Lead = Investimento Meta / Total de Leads. Fonte: Meta API ÷ formularios_anuncios', value: cpl, goal: 150, unit: 'currency' },
         sal: { id: 'sal', label: 'SAL', description: 'Sales Accepted Leads = Deals no Filtro 1 em diante + Vendas. Fonte: Supabase → Deal Alterada (stage_id >= 7)', value: sal, goal: 571, unit: 'number' },
         sql: { id: 'sql', label: 'SQL', description: 'Sales Qualified Leads = Deals no Filtro 2 em diante + Vendas. Fonte: Supabase → Deal Alterada (stage_id >= 8)', value: sql, goal: 400, unit: 'number' },
         mqls: { id: 'mqls', label: 'MQL (Fat. 70k+)', description: 'Leads com Faturamento ≥ 70k ou ≥ 5 colaboradores (público ideal). Fonte: formularios_anuncios.Faturamento', value: mqlQualified, goal: 571, unit: 'number' },
         cpmql: { id: 'cpmql', label: 'Custo por MQL', description: 'Investimento Meta / MQLs qualificados. Fonte: Meta API ÷ formularios_anuncios', value: costPerMql, goal: 350, unit: 'currency' },
         sales: { id: 'sales', label: 'Vendas Total', description: 'Deals com Status = won no mês (deduplicados). Fonte: Supabase → Deal Alterada (Status=won)', value: totalSales, goal: 20, unit: 'number' },
         revenue: { id: 'revenue', label: 'Faturamento', description: 'Soma dos valores dos deals ganhos. Fonte: Supabase → Deal Alterada (Status=won, campo valor)', value: totalRevenue, goal: 830000, unit: 'currency' },
         ticket: { id: 'ticket', label: 'Ticket Médio', description: 'Faturamento / Vendas Total. Fonte: calculado', value: ticket, goal: 25000, unit: 'currency' },
         connections: { id: 'connections', label: 'Conexões', description: 'Leads que passaram do estágio Oportunidades (= SAL). Fonte: Deal Alterada (stage >= Filtro 1)', value: connections, goal: 300, unit: 'number' },
         meetingsBooked: { id: 'meetingsBooked', label: 'Reuniões Agendadas', description: 'Sessões Estratégicas criadas por SDRs (deduplicadas por nome). Fonte: Supabase → atividade_criada', value: meetingsBooked, goal: 264, unit: 'number' },
         meetingsHeld: { id: 'meetingsHeld', label: 'Reuniões Realizadas', description: 'Sessões Estratégicas confirmadas (deduplicadas por nome, excl. RolePlays). Fonte: Supabase → Atividade Alterada', value: meetingsHeld, goal: 180, unit: 'number' },
         noShowRate: { id: 'noShowRate', label: 'Taxa No-Show', description: 'No-Shows / (Realizadas + No-Shows) × 100. Fonte: Atividade Alterada (type=no_show)', value: noShowRate, goal: 30, unit: 'percentage' },
         cac: { id: 'cac', label: 'CAC', description: 'Custo de Aquisição = Investimento / Vendas Total. Fonte: Meta API ÷ Deal Alterada', value: cac, goal: 6000, unit: 'currency' },
         roas: { id: 'roas', label: 'ROAS', description: 'Return on Ad Spend = Faturamento / Investimento. Fonte: Deal Alterada ÷ Meta API', value: roas, goal: 5, unit: 'number', suffix: 'x' },
         salesInbound: { id: 'salesInbound', label: 'Vendas Inbound', description: 'Vendas de leads Inbound/Ads. Fonte: Deal Alterada + Person Criada.From', value: salesInbound, goal: 15, unit: 'number' },
         salesOutbound: { id: 'salesOutbound', label: 'Vendas Outbound', description: 'Vendas de leads Outbound. Fonte: Deal Alterada + Person Criada.From=Outbound', value: salesOutbound, goal: 5, unit: 'number' },
         conversionMeetingSale: { id: 'conversionMeetingSale', label: 'Conv. Venda/Realizada', description: 'Vendas / Reuniões Realizadas × 100. Fonte: calculado', value: convMeetSale, goal: 20, unit: 'percentage' },
         opportunities: { id: 'opportunities', label: 'Oportunidades', description: 'Deals criados no Pipedrive no mês. Fonte: Supabase → Deal Criado (pipeline_id=2)', value: dealsCreated.length, goal: 500, unit: 'number' },
         marketingSales: { id: 'marketingSales', label: 'Vendas MKT', description: 'Mesmo que Vendas Total. Fonte: Deal Alterada (Status=won)', value: totalSales, goal: 20, unit: 'number' },
         marketingRevenue: { id: 'marketingRevenue', label: 'Faturamento MKT', description: 'Mesmo que Faturamento. Fonte: Deal Alterada (Status=won, campo valor)', value: totalRevenue, goal: 830000, unit: 'currency' },
         noShows: { id: 'noShows', label: 'No-Shows', description: 'Sessões com no-show (deduplicadas por nome). Fonte: Supabase → Atividade Alterada (type=no_show)', value: totalNoShows, goal: 20, unit: 'number' },
         rescheduled: { id: 'rescheduled', label: 'Reagendamentos', description: 'Tentativas de reagendamento (deduplicadas). Fonte: atividade_criada + Atividade Alterada (subject contém "Reagendamento")', value: totalRescheduled, goal: 15, unit: 'number' },
      };
